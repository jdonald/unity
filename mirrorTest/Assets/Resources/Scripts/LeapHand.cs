...
public class LeapHand : MonoBehaviour {
	...
	public void ShowIndex(bool showIndex) {
		GameObject [] joints = m_fingers[1].GetComponent<LeapFinger>().GetJoints();
		GameObject [] langes = m_fingers[1].GetComponent<LeapFinger>().GetLanges();
		for (int i = 0; i < joints.Length; ++i) {
			joints[i].renderer.enabled = showIndex;
		}
		for (int i = 0; i < langes.Length; ++i) {
			langes[i].renderer.enabled = showIndex;
		}
	}
