		...
		for(int i = 0; i < f.Hands.Count; ++i) {
			...
			if (m_hands.TryGetValue(f.Hands[i].Id, out hand)) {
				...
				if (f.Hands[i].GrabStrength > 0.4f) {
					...
					hand.GetComponent<LeapRiggedHand>().m_riggedHand.transform.Find("Bip01").Find("Bones").GetComponent<SkinnedMeshRenderer>().material.color = new Color(0.3f, 0.3f, 0.3f, 1.0f);
				}
			} else {
